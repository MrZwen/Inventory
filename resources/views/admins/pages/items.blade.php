                        ...
                        <td>
                            <div class="flex justify-center gap-2">
                                ...
                                <form action="{{ route('items.destroy.admins', $item->id) }}" method="POST" class="inline-block" id="delete-form-{{ $item->id }}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" onclick="confirmDelete('{{ $item->id }}')" class="block text-white bg-red-500 hover:p-1.5 hover:bg-red-600 font-medium rounded-lg text-md p-1.5 text-center">
